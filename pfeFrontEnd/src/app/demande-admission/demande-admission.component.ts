...
@Component({
  selector: 'app-demande-admission',
  templateUrl: './demande-admission.component.html',
  styleUrls: ['./demande-admission.component.css']
})
export class DemandeAdmissionComponent implements OnInit {

  constructor(private demandeService:DemandeService,private router:Router,private route:ActivatedRoute) { }

AdmissionDemands:Demand[];
  ...
  fetchAllDemands(){
    this.demandeService.getAllDemands().subscribe(
      (response)=>{
        this.AdmissionDemands=response;
        this.demandsLoaded = true; // Set to true when data is loaded
        console.log(this.AdmissionDemands);
